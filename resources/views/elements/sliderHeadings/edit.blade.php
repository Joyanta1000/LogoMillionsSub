<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Update Heading of Slider Info') }}
        </h2>
    </x-slot>
    ...
    <div class=" sm:container py-6 px-5 sm:px-0 flex justify-center items-center">
        ...
        <div class="w-full sm:w-[50rem] mx-3 sm:mx-12">
            ...
            @endif
            <form action="{{ route('slider_heading.update', $obj->id) }}" method="post" enctype="multipart/form-data">
                @method('put')
                @csrf
            ...
            <div class="flex flex-wrap -mx-3 mb-2 mt-2">
                ...
                <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                    @if (empty($obj->deleted_at))
                        <form action="{{ route('slider_heading.destroy', $obj->id) }}" method="post">
                            @method('DELETE')
                            @csrf
                    ...
                    @else
                        <form action="{{ route('slider_heading.restore', $obj->id) }}" method="post">
                            @method('PUT')
                            @csrf
                            <button
