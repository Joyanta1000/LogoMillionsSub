<x-app-layout>
    ...
    <div class=" sm:container py-6 px-5 sm:px-0 flex justify-center items-center">
        ...
        <div class="w-full sm:w-[50rem] mx-3 sm:mx-12">
            ...
            @endif
            <form action="{{ route('order_category_amenities.update', $obj->id) }}" method="post" enctype="multipart/form-data">
                @method('put')
                @csrf
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        ...
                        <select name="order_category_id"
                            class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white"
                            id="grid-name" type="text" placeholder="Logo Ex..." value="">
                        <option value="">Select</option>
                        @foreach($orderCategories as $q)
                        <option value="{{$q->id}}" {{$obj->order_category_id == $q->id ? "selected" : ""}}>{{$q->name}}</option>
                        @endforeach
                        </select>
                        @error('order_category_id')
                    ...
                    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        ...
                        <input name="name"
                            ...
                            id="grid-name" type="text" placeholder="Logo Ex..." value="{{ $obj->name }}">
                        @error('name')
                            <div class="text-sm text-red-600">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="flex flex-wrap -mx-3 mb-6">
                    ...
                    <div class="w-full px-3">
                        ...
                        <textarea name="info"
                            ...
                            id="editor" type="text" placeholder="">{{ $obj->info }}
                    </textarea>
            ...
            <div class="flex flex-wrap -mx-3 mb-2 mt-2">
                ...
                <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                    @if (empty($obj->deleted_at))
                        <form action="{{ route('order_category_amenities.destroy', $obj->id) }}" method="post">
                            @method('DELETE')
                            @csrf
                    ...
                    @else
                        <form action="{{ route('order_category_amenities.restore', $obj->id) }}" method="post">
                            @method('PUT')
                            @csrf
                            <button
